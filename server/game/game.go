...
func (g *Game) GetCurrentMovie() Movie {
	...
	return Movie{Title: g.movies[m.idx], MsToNextMovie: MovieDuration - time.Now().Sub(m.currentStartTime).Nanoseconds()/int64(time.Millisecond)}
}
...
func (g *Game) GuessTitle(playerName string, titleName string) string {
	...
	m := g.currentMovie.Load().(movie)
	var message string
	if strings.EqualFold(g.movies[m.idx], titleName) {
